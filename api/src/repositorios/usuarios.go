...
// BuscarPorEmail busca um usuário pelo email e retorna o seu id e senha com hash.
...
// BuscarSeguindo traz todos usuários que um determinado usuário está seguindo.
func (repositorio Usuarios) BuscarSeguindo(usuarioID uint64) ([]modelos.Usuario, error) {
	linhas, erro := repositorio.db.Query(`
	select u.id, u.nome, u.nick, u.email, u.criadoEm from usuarios u inner join seguidores s on u.id = s.usuario_id where s.seguidor = ?
	`, usuarioID)
	if erro != nil {
		return nil, erro
	}
...
func (repositorio Usuarios) BuscarSenha(usuarioID uint64) (string, error) {
	linhas, erro := repositorio.db.Query("select senha from usuarios where id = ?")
	if erro != nil {
		return "", erro
	}
	defer db.Close()
